...
function eventsProtocolFactory (
    ...
    EventsProtocol.prototype.publishTaskNotification = function (nodeId, taskId, graphId, type, data) {
        assert.string(nodeId);
        assert.uuid(taskId);
        assert.uuid(graphId);
        assert.string(type);
        ...
        if(typeof data !== 'string' && typeof data !== 'object') {
            throw new Error("Bad Notification Data");
        }

        return messenger.publish(
            Constants.Protocol.Exchanges.Events.Name,
            'task.notification',
            { nodeId: nodeId, taskId: taskId, graphId: graphId, type: type, data: _data }
        );
    };

    EventsProtocol.prototype.subscribeTaskNotification = function (callback) {
        assert.func(callback);
