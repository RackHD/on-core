...
function taskProtocolFactory (Q, assert, Constants, messenger, _, Result) {
    ...
    TaskProtocol.prototype.publishRunIpmiSdrCommand = function (routingKey, data) {
        assert.uuid(routingKey);
        assert.object(data);

        return messenger.publish(
            Constants.Protocol.Exchanges.Task.Name,
            'run.ipmi.sdr.command' + '.' + routingKey,
            new Result({ value: data })
        );
    };

    TaskProtocol.prototype.subscribeRunIpmiSdrCommand = function (routingKey, callback) {
        assert.uuid(routingKey, 'routingKey');
        assert.func(callback, 'callback');
    ...
    TaskProtocol.prototype.subscribeIpmiSdrCommandResult = function (routingKey, callback) {
        assert.uuid(routingKey, 'routingKey');
        assert.func(callback, 'callback');

        return messenger.subscribe(
            Constants.Protocol.Exchanges.Task.Name,
            'ipmi.sdr.command.result' + '.' + routingKey,
            callback
        );
    };

    TaskProtocol.prototype.publishIpmiSdrCommandResult = function (routingKey, results) {
        assert.uuid(routingKey);
        assert.object(results);
    ...
    TaskProtocol.prototype.publishRunSnmpCommand = function (routingKey, data) {
        assert.uuid(routingKey);
        assert.object(data);

        return messenger.publish(
            Constants.Protocol.Exchanges.Task.Name,
            'run.snmp.command' + '.' + routingKey,
            new Result({ value: data })
        );
    };

    TaskProtocol.prototype.subscribeRunSnmpCommand = function (routingKey, callback) {
        assert.uuid(routingKey, 'routingKey');
        assert.func(callback, 'callback');
    ...
    TaskProtocol.prototype.subscribeSnmpCommandResult = function (routingKey, callback) {
        assert.uuid(routingKey, 'routingKey');
        assert.func(callback, 'callback');

        return messenger.subscribe(
            Constants.Protocol.Exchanges.Task.Name,
            'snmp.command.result' + '.' + routingKey,
            callback
        );
    };

    TaskProtocol.prototype.publishSnmpCommandResult = function (routingKey, results) {
        assert.uuid(routingKey);
        assert.object(results);
    ...
    TaskProtocol.prototype.publishPollerAlert = function (routingKey, results) {
        assert.uuid(routingKey);
        assert.object(results);

        return messenger.publish(
            Constants.Protocol.Exchanges.Events.Name,
            'poller.alert' + '.' + routingKey,
            new Result({ value: results })
        );
    };
