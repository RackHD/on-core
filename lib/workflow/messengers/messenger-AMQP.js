...
function amqpMessengerFactory(
    ...
    exports.publishProgressEvent = function(graphId, data) {
        ...
        .then(function(graph){
            ...
            if (graph.tasks) {
                data.progress.value = _countNonPengingTask(graph.tasks);
                data.progress.maximum = _.size(graph.tasks);
            }
            if (graph._status === Constants.Task.States.Succeeded) {
                data.progress.value = data.progress.maximum;
            }
    ...
    function _countNonPengingTask(tasks) {
