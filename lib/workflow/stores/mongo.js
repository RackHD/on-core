...
function mongoStoreFactory(waterline, Promise, Constants, Errors, assert, _, sanitizer) {
    ...
    */
    ...
    exports.updateGraphProgress = function(data) {
        ...
        return waterline.graphobjects.find({instanceId: data.graphId})
        .then(function(graphObject){
            if (!_.has(data, 'graphName')) {
                data.graphName = graphObject[0].definition.friendlyName;
            }
            //graph progres percentage might not change when task prrogress
            //is updating
            if (data.progress.percentage === "na" && 
                _.has(graphObject[0], "progress.percentage")) {
                data.progress.percentage = graphObject[0].progress.percentage;
            }
            update = {
                $set: { 
                    progress: {
