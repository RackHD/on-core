    ...
    */
    ...
    exports.updateDependentTasks = function(data) {
        ...
        return waterline.taskdependencies.find(query).then(function(res) {
           if (_.isEmpty(res)) {
                // no task found under dependencies try search either
                delete query['dependencies.' + data.taskId];
                query['dependencies.either.' + data.taskId] = {
                    $in: [data.state, Constants.Task.States.Finished]
                };

                delete update.$unset['dependencies.' + data.taskId];
                update.$unset['dependencies.either'] = '';
     ...
     * @param {Object} data - data includes graphId, progress message
     * @returns {Promise} a promise with modified percentage info of next graph progress data 
     * @memberOf store
     */
    exports.updateGraphProgress = function(data) {
        assert.uuid(data.graphId, "Graph ID should be a UUID");
        assert.string(data.progress.description, "Progress info should include description");
        assert.string(data.progress.percentage, "Progress info should include percentage data");
        var update;
        var query = {
            instanceId: data.graphId,
            _status: {$in: Constants.Task.ActiveStates}
        ...
        return waterline.graphobjects.findOne({instanceId: data.graphId})
        .then(function(graphObject){
            if (!_.has(data, 'graphName')) {
                data.graphName = graphObject.definition.friendlyName;
            }
            //graph progress percentage might not change when task prrogress
            //is updating
            if (data.progress.percentage === "Not Available" &&
                _.has(graphObject, "progress.percentage")) {
                data.progress.percentage = graphObject.progress.percentage;
            }
            update = {
                $set: { 
                    progress: {
        ...
        .tap(function(){
            return waterline.graphobjects.updateMongo(query, update, options);
        });
    };
    
    /**
     * Update task progress
     * @param {Object} data - data includes graphId, progress message and taskId
     * @returns {Promise} 
     * @memberOf store
     */
    exports.updateTaskProgress = function(data) {
        assert.uuid(data.graphId, "Graph ID should be a UUID");
        assert.uuid(data.taskId, "Task ID should be a UUID");
        assert.object(data.progress, "Progress info should be an object");
        assert.string(data.progress.description, "Progress info should include description");
        assert.string(data.progress.percentage, "Progress info should include percentage data");
