...
function mongoStoreFactory(waterline, Promise, Constants, Errors, assert, _) {
    ...
    */
     ...
     * @param {String} isNew - Boolean, when true a create message is broadcast.
     * @returns {Promise} a promise for the persisted graph object
     * @memberOf store
     */
    exports.persistGraphObject = function(graph, isNew) {
        ...
        var promise = waterline.graphobjects.findAndModifyMongo(query, {}, graph, options);

        if (isNew) {
            promise.then(function () {
                waterline.graphobjects.publishRecord('created', graph);
            });
        }
