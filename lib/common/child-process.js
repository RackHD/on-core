...
di.annotate(ChildProcessFactory,
    new di.Inject(
        'Q', 'Logger', 'Assert', '_', 'child_process'
    )
);
 ...
 * @private
 */
function ChildProcessFactory (Q, Logger, assert, _, nodeChildProcess) {
    ...
    ChildProcess.prototype.run = function run (command, args, env, code) {
        ...
        if (file) {
            self.spawnInstance = nodeChildProcess.execFile(file, args, environment,
