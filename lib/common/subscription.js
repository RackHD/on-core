...
function SubscriptionFactory (assert) {
    function Subscription (messenger, options) {
        assert.ok(messenger, 'messenger');

        this.messenger = messenger;

        assert.ok(options, 'options');
        assert.ok(options.queue, 'options.queue');
        assert.ok(options.consumerTag, 'options.consumerTag');
    ...
    Subscription.prototype.dispose = function () {
        ...
        return self.messenger.cancel(self.consumerTag).then(function () {
            return self.messenger.deleteQueue(self.queue);
        });
    };
