 ...
 * * @param  {nconf} nconf Configuration Instance.
 * * @param  {Q} Q Q Instance.
 * * @param  {di.Injector} injector Dependency Injector.
 * @return {Logger} a Logger instance.
 */
function loggerFactory(Constants, nconf, Q, injector) {
    var levels = {
        emerg: 8,
        alert: 7,
        crit: 6,
    ...
    Logger.prototype._translate = function(args) {
        ...
        if (args.meta.macaddress) {
            return injector.get('lookup-service')
    ...
    Logger.prototype._filtered = function (level) {
        var configured = levels[(nconf.get('level') || 'silly')],
