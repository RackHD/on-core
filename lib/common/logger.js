 ...
 */
function loggerFactory(Constants, loggingProtocol, configuration, Q, _, injector) {
    var levels = {
        emerg: 8,
        alert: 7,
        crit: 6,
    ...
    Logger.prototype._publish = function(args) {
        ...
        loggingProtocol.publishLog(data.level, data);
    ...
    // Make this a public utility method that can be called with a logger
    // instance or just the constructor. Ugly, but the alternative is to make
    // a new file/service just for this function...
    Logger.write = function write(args, context) {
        if (args.caller) {
        ...
        console.log(
            ...
            (configuration.get('color') ? colors[args.level](args.level) : args.level) +
            ' ' + args.message
        );
    ...
    Logger.prototype.write = Logger.write;
