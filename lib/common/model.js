...
function ModelFactory (_, assert, Waterline, waterlineProtocol) {
    ...
    return Waterline.Collection.extend({
        ...
        afterDestroy: function (records, next) {
            records.forEach(
              waterlineProtocol.publishRecord.bind(waterlineProtocol, this, 'destroyed')
            );
            next();
        },
        ...
        /**
         * Find or Create a Single Document based on the criteria and data.
         * @param  {Object} criteria Waterline Criteria Object
         * @param  {Object} data Object used for the create/update action.
         * @return {Deferred}
         */
        findOrCreate: function (criteria, data) {
            var self = this;

            return this.findOne({ where: criteria }).then(function (doc) {
                if (doc) {
                    return self.update({ where: criteria }, data);
                } else {
