...
function WaterlineServiceFactory(
    ...
    WaterlineService.prototype.start = function () {
        ...
        return new Promise(function (resolve, reject) {
            ...
            assert.ok(self.config.connections.mongo, 'mongo');
            assert.ok(self.config.connections.mongo.host, 'mongo.host');
            assert.ok(self.config.connections.mongo.port, 'mongo.port');
            assert.ok(self.config.connections.mongo.database, 'mongo.database');
    ...
    WaterlineService.prototype.stop = function () {
        if (this.initialized) {
            return Promise.fromNode(this.service.teardown.bind(this.service));
        } else {
