...
function WaterlineServiceFactory(
    ...
    WaterlineService.prototype.subscribe = function (deferred, callback) {
        assert.object(deferred);
        ...
        if (!_.isObject(deferred._context)) {
            collection = deferred;
        } else if (_.isObject(deferred._criteria)) {
            query = deferred._criteria.where;
        }

        return collection.subscribe(query, callback);
    };
