 ...
 */
function lookupServiceFactory(Q, configuration, dhcpProtocol, waterline, assert, Errors) {
    ...
    LookupService.prototype.ipAddressToMacAddress = function ipAddressToMacAddress(ip) {
        ...
        return dhcpProtocol.lookupIpLease(ip).then(function (macAddress) {
            cache.set(ip, macAddress);
            return macAddress;
        });
    };
    ...
    LookupService.prototype.macAddressToNode = function macAddressToNode(macAddress) {
        // TODO: add assert.isMacAddress
        assert.ok(macAddress, 'macAddress');
    ...
    LookupService.prototype.macAddressToNodeId = function macAddressToNodeId(macAddress) {
        ...
        return self.macAddressToNode(macAddress).then(function(node) {
            if (node) {
                ...
                return Q.reject(new Errors.LookupError('Unable to locate node via MAC address.'));
            }
        });
    };
