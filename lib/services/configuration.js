...
function configurationServiceFactory(
    ...
    ConfigurationService.prototype.load = function() {
        ...
        logger.info('Setting base directory for loading configuration to %s.'.format(baseDirectory));
    };
    ...
    ConfigurationService.prototype.getAll = function () {
        if (!this.started) {
            logger.deprecate(
                'Attempting to access configuration values prior to configuration service being started.', 3
            );
        }
